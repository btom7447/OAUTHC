...
import { ClipLoader } from "react-spinners"; // Import the preloader

const DepartmentsFocusArea = () => {
  const departments = useDepartments();
  const [isLoading, setIsLoading] = useState(true); // State to manage loading

  useEffect(() => {
    if (departments.length === 0) {
      setIsLoading(true); // Set loading to true if no data
    } else {
      setIsLoading(false); // Set loading to false once data is available
    }
  }, [departments]);

  // Sort the departments array in alphabetical order by departmentName
  const sortedDepartments = [...departments].sort((a, b) =>
    a.departmentName.localeCompare(b.departmentName)
  );

  return (
    <div className="departments-f-areas-section">
      {isLoading ? (
        ...
        <>
          <div className="d-f-caption">
            <h5>We Solve the World's most Serious</h5>
            <h3>Departments and Focus Areas</h3>
          </div>
...
export default DepartmentsFocusArea;
